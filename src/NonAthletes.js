...
import { createAthleteNonAthlete } from "./graphql/mutations"; // Assuming mutation is correctly defined and exported
...
function NonAthlete() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      console.log(
        "NonAthlete created:",
        newNonAthlete.data.createNonAthlete.id
      );
      for (const element of athlete) {
        const athleteLinks = athlete.map((element) => {
          let athleteDetail = {
            athleteID: element,
            nonAthleteID: newNonAthlete.data.createNonAthlete.id,
          };
          return client.graphql({
            query: createAthleteNonAthlete,
            variables: { input: athleteDetail },
          });
        });

        try {
          const results = await Promise.all(athleteLinks);
          results.forEach((result) => console.log("Relation created:", result));
        } catch (err) {
          console.error("Error creating relations:", err);
          setError("Failed to create one or more relations");
        }
      }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button type="submit" disabled={loading}>
        {loading ? "Processing..." : "Add NonAthlete"}
      </button>
    </form>
  );
}
